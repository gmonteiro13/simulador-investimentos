...
def calculate_annualized_volatility(series):
    ...
    volatility = daily_returns.std() * np.sqrt(252) # número aprox. de dias de negociação
    return volatility
...
def calculate_sharpe_ratio(series, risk_free_rate=0.0):
    ...
    if volatility == 0: return np.inf # caso retorno não tenha risco

    sharpe = (annualized_return - risk_free_rate) / volatility
    return sharpe

def calculate_all_metrics(series, risk_free_cagr=0.0):
    """Calcula todas as métricas para uma dada série de valores e retorna um dicionário."""
