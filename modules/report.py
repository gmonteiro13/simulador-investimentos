...
def generate_comparison_plot(
    ...
    ax.plot(interest_series.index, interest_series, label=interest_series.name, color='royalblue', linewidth=2)
    ax.plot(portfolio_series.index, portfolio_series, label=portfolio_series.name, color='darkorange', linewidth=2)
    
    # Formatando o eixo Y para parecer com moeda
    formatter = mticker.FuncFormatter(lambda x, p: f'R$ {x:,.0f}'.replace(',', '.'))
...
def generate_drawdown_plot(portfolio_series: pd.Series):
    ...
    formatter = mticker.FuncFormatter(lambda x, p: f'{x:.1%}')
    ax.yaxis.set_major_formatter(formatter)
...
def generate_report_table(
    ...
    df['Volatilidade Anual. (%)'] = df['Volatilidade Anual. (%)'].map('{:.2f}%'.format)
