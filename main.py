...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Simulação de Juros Compostos vs. Carteira de Ações."
    )
    ...
    parser.add_argument('--aporte_mensal', type=float, required=True)
    parser.add_argument('--capital_inicial', type=float, required=True)
    parser.add_argument('--taxa_juros_mensal', type=float, required=True)
    ...
    interest_series = simulate_compound_interest(
        args.capital_inicial,
        args.aporte_mensal,
        args.taxa_juros_mensal,
        price_data.index
    )
    ...
    portfolio_metrics = calculate_all_metrics(portfolio_series,
                                              risk_free_cagr=interest_cagr)
